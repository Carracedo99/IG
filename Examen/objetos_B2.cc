...
_objeto_examen1::_objeto_examen1(float tam, float tam2, float al)
{

//vertices 
vertices.resize(6); 
vertices[0].x=-tam*2;vertices[0].y=-tam;vertices[0].z=tam2;
vertices[1].x=tam*2;vertices[1].y=-tam;vertices[1].z=tam2;
vertices[2].x=tam;vertices[2].y=tam;vertices[2].z=0;
vertices[3].x=-tam;vertices[3].y=tam;vertices[3].z=0;

vertices[4].x=tam*2;vertices[4].y=-tam;vertices[4].z=-tam2;
vertices[5].x=-tam*2;vertices[5].y=-tam;vertices[5].z=-tam2;
...
caras[5]._0=5;caras[5]._1=0;caras[5]._2=3;

//caras de abajo
caras[6]._0=4;caras[6]._1=1;caras[6]._2=0;
caras[7]._0=0;caras[7]._1=5;caras[7]._2=4;
...
_objeto_examen2::_objeto_examen2(float radio1, float radio2, int num1, int lados){
  ...
  int i;
  		...
  		aux.x=0.0; aux.y=0.0; aux.z=0.0;
			perfil.push_back(aux);
			aux.x=0.3; aux.y=0.0; aux.z=0.0;
			perfil.push_back(aux);



for (i=0;i<=num1/2;i++)
  { aux.x=radio1*cos(M_PI*i/(num1*1.0)-M_PI/2.0)+0.5;
    aux.y=radio1*sin(M_PI*i/(num1*1.0)-M_PI/2.0)+0.5;
    aux.z=0.0;
    perfil.push_back(aux);
  }


  		aux.x=0.8; aux.y=0.5; aux.z=0.0;
			perfil.push_back(aux);
  		aux.x=0.3; aux.y=0.5; aux.z=0.0;
			perfil.push_back(aux);
			aux.x=0.3; aux.y=1.0; aux.z=0.0;
			perfil.push_back(aux);
...
crear_cabeza.draw(modo, r, g, b, grosor);
glPopMatrix();
};
...
glPushMatrix();
glScalef(ancho, alto, fondo);
crear_cola.draw(modo, r, g, b, grosor);
glPopMatrix();
};
...
//PRIMERO TRANSLACION SEGUNDO ROTACION  --> SEGURO??
...
void _pez::draw(_modo modo, float r, float g, float b, float grosor)
{
  ...
  // TRONCO DEL PEZ Y CABEZA------------------------------------------------------------    ARREGLAR ORDEN!!!!
...
//************************************************************************
// objeto examen3 (montaje del objeto final)
//************************************************************************

_objeto_examen3::_objeto_examen3()
{
...
void _objeto_examen3::draw(_modo modo, float r, float g, float b, float grosor)
{
  ...
  glPushMatrix();
  glRotatef(-30,0,0,1);
  glScalef(1,1+escalado1,1);
  glTranslatef(0,1.2,0);
  cilindro.draw(modo, r, g, b, grosor);
  glPopMatrix();


  glTranslatef(1.3+escalado1,1.7+escalado1,0);
