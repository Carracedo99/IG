...
_pez::_pez()
{
 ...
 giro_aleta2_min = -65.0;
  ...
  int partes = 5;
  int color=80;
  color_select.resize(partes);
  activo.resize(partes);
  grosor_select=2;
...
void _pez::draw(_modo modo, float r, float g, float b, float grosor)
{
  ...
  if (activo[2]==1){
    ...
    cola.draw(EDGES, r_p, g_p, b_p, grosor_select);
    glPopMatrix();
  ...
  if (activo[3]==1){
    ...
    aletas.draw(EDGES, r_p, g_p, b_p, grosor_select);
    glPopMatrix();
...
glPushMatrix();
  ...
  color=color_select[3];
    ...
    glRotatef(giro_aleta1,0,0,1);
    glScalef(0.1,1.5,0.5);           
    aletas.draw(SELECT, color.r, color.g, color.b, 1);
    glPopMatrix();
